...
mochaShot.createTest = function(fileName, filePath) {
  if (fileName === undefined || filePath === undefined || typeof fileName !== 'string' || typeof filePath !== 'string') return console.log('Error: Must provide module name and path as a string (i.e. mochaShot.createTest("module_name", "file_path")')
  
  return fs.access('../test/test.js', fs.F_OK, function(err) {
    if (!err) {
      var requirePath = "\nvar "+fileName+" = require("+JSON.stringify(filePath)+");\n"
      var escapedString = escapeStringRegexp(requirePath);
      return fs.readFile('../test/test.js', 'utf8', function(err, data) {  
...
mochaShot.equal = function(des, input, expected_output) {
  ...
  var escapedString = escapeStringRegexp(test);
  
  fs.readFile('../test/test.js', 'utf8', function(err, data) {  
    if (err) {
